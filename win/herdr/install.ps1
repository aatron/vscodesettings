...
function Install-Shim([string]$Src, [string]$Dest) {
  ...
  # Prefer hardlink/symlink when possible; fall back to copy.
  ...
  try {
    ...
    try {
      ...
      Write-Host "-> shim:  $Dest -> $Src (hardlink)"
    } catch {
      Copy-Item -LiteralPath $Src -Destination $Dest -Force
      Write-Host "-> shim:  $Dest <- $Src (copy)"
    }
  }
}
...
Install-File (Join-Path $ScriptDir 'new-worktree-dev-windows.toml') (Join-Path $QaDir 'new-worktree-dev-windows.toml')
